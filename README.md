...
The IP address of the microcontroller can be found using the administration page of the connected router.
Otherwise the message can only be changed by reflashing the microcontroller with a different hardcoded message.

### Sending
The mikrocontroller will continuously send the message in a loop while it is powered.

## Decoder
### Hardware
The code runs on a linux device with a camera descriptor on `/dev/video0`.
### Configuration
The camera is configured via `make camera_config` which uses `cameractrls` as a dependency.
### Compilation
