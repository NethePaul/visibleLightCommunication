...
bool decodeFrame(BitStream& stream, double sub_delta, size_t frame_id, bool rising, cv::Point loc, double v)
{
    ...
    if (v > stream.last_v && stream.has_unprocessed_bit && ((rising && stream.rising == stream.RISING) || (!rising && stream.rising == stream.FALLING))) {
    ...
    if (
        (rising && stream.rising != stream.RISING) || (!rising && stream.rising != stream.FALLING)) {
        decodeBit(stream, sub_delta, delta, frame_id, loc, v, rising);
...
int main(int argc, const char** argv, const char** env)
{
    ...
    if (argc >= 3) {
        min_thresh = std::atoi(argv[2]);
    }
    if (argc >= 4) {
        max_thresh = std::atoi(argv[3]);
    }
    if (argc >= 5) {
        neighbourhood = std::atoi(argv[4]);
    }
